...
import { CheckCircle, Lightbulb, FileArrowDown, FilePdf, CircleNotch } from "@phosphor-icons/react";
import { generatePdfReport } from "@/lib/pdfGenerator";
import { SingleAuditPdfTemplate } from "./PdfReportTemplate";
...
export function EstimateCheckReport({
  ...
  const handleExportPdf = async () => {
    ...
    try {
      await generatePdfReport(pdfTemplateRef.current, report.summary.fileName);
    } catch (error) {
    ...
    <>
      ...
      <div style={{ position: "absolute", left: "-9999px", top: "-9999px" }}>
        <div ref={pdfTemplateRef}>
          <SingleAuditPdfTemplate report={report} lines={lines} vatRate={vatRate} />
        </div>
      </div>
    </>
  );
